...
export class OrdersComponent implements OnInit {
  ...
  availableStatuses: string[] = ['PENDING', 'PROCESSING', 'SHIPPED', 'DELIVERED', 'CANCELLED', 'RETURNED'];
  ...
  loadOrders(page: number): void {
    this.isLoading = true;
    this.orderService.getAllOrdersPaginated(page, this.pageSize).subscribe({
      next: (res) => {
  ...
  onStatusChange(order: OrderResponseDTO, newStatus: string): void {
    // update local model
    ...
    this.orderService.updateOrderStatus(order.orderId, newStatus).subscribe({
      next: () => {
        console.log('Order status updated!', order.orderId, newStatus );
      },
      error: (err) => {
